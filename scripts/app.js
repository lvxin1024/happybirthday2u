...
const birthdayName = recipient.name || "小星";
const birthdayAge = recipient.age || 18;
const birthdayDate = recipient.birthdayDate || "2027-01-01";
const unlockAt = Date.parse(`${birthdayDate}T00:00:00+08:00`);
const photoMemories = config.photos || [];
const fandomMessages = Object.fromEntries(
  (config.interests || []).map((interest) => [interest.key, interest])
);
const gachaBlessings = config.gachaBlessings || [];
...
function renderEditableContent() {
  document.title = config.meta?.title || "生日奇想剧场";
  const description = document.querySelector('meta[name="description"]');
  if (description && config.meta?.description) {
    description.content = config.meta.description;
  }

  document.querySelectorAll("[data-recipient-name]").forEach((element) => {
    element.textContent = birthdayName;
  });
  document.querySelector("[data-birthday-heading]").textContent = `${birthdayAge}岁生日快乐！`;
  document.querySelector("[data-cake-message]").textContent = config.hero?.cakeMessage || "生日快乐！";

  const [year, month, day] = birthdayDate.split("-");
  document.querySelector("[data-lock-copy]").textContent =
    `时间到达 ${year} 年 ${Number(month)} 月 ${Number(day)} 日 00:00 前，生日剧场会安静等候。`;

  const floatingWishes = (config.hero?.floatingWishes || []).map((wish) => createElement("span", "", wish));
  document.querySelector("[data-floating-wishes]").replaceChildren(...floatingWishes);

  const photoCards = photoMemories.map((memory, index) => {
    const card = createElement("article", `memory-polaroid magnetic ${memory.cardClass || ""}`);
    card.dataset.photoId = String(index);
    card.tabIndex = 0;
    card.setAttribute("role", "button");
    card.setAttribute("aria-label", `${memory.title}，${memory.date}`);
    const tape = createElement("span", `photo-tape ${memory.tapeClass || "tape-left"}`);
    tape.setAttribute("aria-hidden", "true");
    const sticker = createElement("span", "photo-sticker", memory.sticker || "✦");
    sticker.setAttribute("aria-hidden", "true");
    const image = createElement("img");
    image.src = memory.image;
    image.alt = memory.alt || "回忆插画";
    image.loading = index === 0 ? "eager" : "lazy";
    const meta = createElement("div", "polaroid-meta");
    meta.append(
      createElement("p", "photo-date", memory.date),
      createElement("p", "photo-location", memory.location)
    );
    card.append(tape, sticker, image, meta);
    return card;
  });
  document.querySelector("[data-photo-board]").replaceChildren(...photoCards);

  const planets = (config.interests || []).map((interest) => {
    const button = createElement("button", `planet magnetic ${interest.planetClass || ""}`);
    button.type = "button";
    button.dataset.fandom = interest.key;
    button.append(createElement("span", "planet-name", interest.label));
    return button;
  });
  document.querySelector(".universe-halo").after(...planets);

  document.querySelector("[data-letter-label]").textContent = config.letter?.label || "一封生日来信";
  const letterLines = (config.letter?.lines || []).map((line) => createElement("p", "letter-line", line));
  document.querySelector("[data-letter-lines]").replaceChildren(...letterLines);
  document.querySelector("[data-finale-title]").textContent = config.finale?.title || "生日快乐";
  document.querySelector("[data-finale-body]").textContent = config.finale?.body || "愿新一岁一切顺利。";
}
...
const gestureButton = document.querySelector("#enable-gestures");
...
const wishes = config.ambientWishes || ["生日快乐"];
...
function checkGate() {
  updateBeijingClock();
  if (config.previewMode || Date.now() >= unlockAt) {
    unlockExperience();
  } else {
...
async function requestCameraPermissionFirst() {
  if (window.__saturnCameraStream) {
    showToast("手势互动已经开启。");
    return;
  }
  ...
  if (!navigator.mediaDevices?.getUserMedia) {
    loadSaturnFrame();
    showToast("当前浏览器不支持摄像头互动。");
    return;
  }

  gestureButton?.setAttribute("disabled", "");
  ...
  try {
    ...
    gestureButton.textContent = "手势互动已开启";
    showToast("摄像头只用于本地手势识别，不会上传画面。");
  } catch (error) {
    syncSaturnHandScale(1);
    gestureButton?.removeAttribute("disabled");
    showToast("未获得摄像头权限，仍可拖动星云浏览。");
  } finally {
    loadSaturnFrame();
  }
}
...
function openPhotoModal(photoId, sourceCard) {
  ...
  photoModalImage.src = image?.getAttribute("src") || "./assets/illustrations/memory-cake.svg";
  photoModalImage.alt = image?.getAttribute("alt") || "放大的回忆插画";
...
function openFandomModal(key, sourcePlanet) {
  ...
  fandomWishes.replaceChildren(...data.wishes.map((wish) => createElement("p", "", wish)));
  if (fandomMedia) {
    const images = Array.isArray(data.image)
      ? data.image
      : [data.image || "./assets/illustrations/interest-universe.svg"];
    const alts = Array.isArray(data.imageAlt) ? data.imageAlt : [data.imageAlt || `${data.label} 插画`];
    const mediaElements = images.map((src, index) => {
      const image = createElement("img", "fandom-image");
      image.src = src;
      image.alt = alts[index] || alts[0];
      image.loading = "lazy";
      return image;
    });
    fandomMedia.replaceChildren(...mediaElements);
  }
...
gestureButton?.addEventListener("click", requestCameraPermissionFirst);
